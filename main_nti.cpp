...
int main (int argc, char *argv[]) {
	...
	fprintf (fstd_log, "FName\tUnixtime\tLon\tLat\tSamp\tLine\tNTI\t#Stdvs\tb221\tNTI_mean\tNTI_stdv\tdist\tSolZenith\r\n") ;
	...
	geo->init_MOD03() ;
/*
        if (this_mday > 1 && this_mday < 210 || this_mday > 245) {
		delete therm ;
		delete geo ;
		continue ;
		}
	...
	// get nti mean and stddev for each pixel in the modis image
	sinu->set_mday (this_mday) ;
	sinu->get_mn_stdev_array_nti (lat, lon, 1354L*2030, modisflag, mnstdv) ;
	...
	// 0 is nti hits
	int num_std_hits = al->calc_nti_stdv (b21, b22, b32, mnstdv, al_std, al_std_inds, al_std_vals) ;
