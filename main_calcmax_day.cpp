...
int main (int argc, char *argv[]) {
    ...
    float temp22, temp32, medval, centval ;
    int i, j, ns, nl, npts, maxsub ;
	float inarr[9], max22, max32 ;
    FILE *fin, *fout ;	
    if (argc < 3) {
	...
	//strcpy (prefix, *++argv) ;
                    ...
                    for (i=0; i<ns * nl; i++) {
                        ...
                        for (j=0; j<npts; j++) {
							...
							//as its better to be a higher value, do not exclude unless necc
                            //if (temp_diff[j] > 45.) continue ;
					...
					for (i=0; i<nl; i++) {
						for (j=0; j<ns; j++) {
							...
							// now filter
							medval = fmedian (inarr, count) ;
							maxarr[i*ns+j] = medval ;
							// only put the new value in if median >
							// current value
							//if (medval > centval) 
							//	maxarr[i * ns + j] = medval ;
						}
